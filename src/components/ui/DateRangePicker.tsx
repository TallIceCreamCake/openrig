...
const formatForStorage = (date: Date, boundary: 'start' | 'end') => {
  const base = boundary === 'start' ? startOfDay(date) : endOfDay(date);
...
const DateRangePicker: React.FC<DateRangePickerProps> = ({
  ...
  const [selectedEnd, setSelectedEnd] = useState<Date | null>(() => toDisplayDate(end));
  const [currentMonth, setCurrentMonth] = useState<Date>(() => startOfMonth(toDisplayDate(start) || new Date()));
  const [hoveredDate, setHoveredDate] = useState<Date | null>(null);
  const ref = useRef<HTMLDivElement | null>(null);
  const hasSingleDayToggle = typeof onSingleDayChange === 'function';
  const singleDayLabelText = singleDayLabel || '1 jour';

  useEffect(() => {
    setSelectedStart(toDisplayDate(start));
    setSelectedEnd(toDisplayDate(end));
    if (start) {
      setCurrentMonth(startOfMonth(toDisplayDate(start) || new Date()));
    }
  }, [start, end]);
  ...
  const emitChange = useCallback((startDate: Date | null, endDate: Date | null) => {
    onChange({
      start: startDate ? formatForStorage(startDate, 'start') : undefined,
      end: endDate ? formatForStorage(endDate, 'end') : undefined,
    });
  }, [onChange]);

  const commitSingleDaySelection = useCallback(() => {
    if (!selectedStart || selectedEnd) return;
    setSelectedEnd(selectedStart);
    emitChange(selectedStart, selectedStart);
  }, [selectedStart, selectedEnd, emitChange]);

  useEffect(() => {
    const handler = (event: MouseEvent) => {
      if (!open && !optionsOpen) return;
      if (ref.current && !ref.current.contains(event.target as Node)) {
        commitSingleDaySelection();
        setOpen(false);
  ...
  }, [open, optionsOpen, commitSingleDaySelection]);
  ...
  const handleDaySelect = (day: GridDay) => {
    ...
    if (!selectedStart || (selectedStart && selectedEnd)) {
      ...
      setSelectedEnd(null);
      emitChange(dayDate, null);
      return;
    }

    if (isSameDay(dayDate, selectedStart)) {
      setSelectedEnd(dayDate);
      emitChange(dayDate, dayDate);
      return;
    }

    if (isBefore(dayDate, selectedStart)) {
      setSelectedStart(dayDate);
      setSelectedEnd(selectedStart);
      emitChange(dayDate, selectedStart);
      return;
    }

    setSelectedEnd(dayDate);
    emitChange(selectedStart, dayDate);
  };
  ...
  const displayStart = selectedStart ? formatLabel(selectedStart) : '';
  const displayEnd = selectedEnd ? formatLabel(selectedEnd) : '';
  const basePlaceholder = placeholder === 'Sélectionner une période' ? 'JJ/MM/AAAA' : placeholder;
      ...
      <div className="relative">
        ...
        {open && (
          <div className="absolute z-30 mt-2 w-[320px] rounded-2xl border border-gray-200 bg-white shadow-xl">
            ...
            <div
              className="grid grid-cols-7 gap-y-1 px-5 pb-4"
            ...
            >
              {gridDays.map((day, idx) => {
                ...
                const dayDate = startOfDay(day.date);
                const hasHoverRange = selectedStart && hoveredDate && !selectedEnd && !isSameDay(hoveredDate, selectedStart);
                const effectiveRangeEnd = selectedEnd || (hasHoverRange ? hoveredDate : null);
                const rangeStart = selectedStart && effectiveRangeEnd && isBefore(effectiveRangeEnd, selectedStart)
                  ? effectiveRangeEnd
                  : selectedStart;
                const rangeEnd = selectedStart && effectiveRangeEnd && isBefore(effectiveRangeEnd, selectedStart)
                  ? selectedStart
                  : effectiveRangeEnd;
                const inRange = rangeStart && rangeEnd && isBefore(rangeStart, dayDate) && isBefore(dayDate, rangeEnd);
                const isRangeStart = rangeStart && isSameDay(dayDate, rangeStart);
                const isRangeEnd = rangeEnd && isSameDay(dayDate, rangeEnd);
                const isSingleSelection = selectedStart && !effectiveRangeEnd && isSameDay(dayDate, selectedStart);
                ...
                return (
                  <div key={`${day.date.toISOString()}-${idx}`} className={wrapperClasses.join(' ')}>
                    <button
                      ...
                      onMouseEnter={() => {
                        if (selectedStart && !selectedEnd) setHoveredDate(dayDate);
                      }}
                      disabled={isDisabled}
        ...
        {optionsOpen && hasSingleDayToggle && (
          <div className="absolute left-0 right-0 z-30 mt-2">
            <div className="rounded-2xl border border-gray-200 bg-white shadow-xl origin-top animate-float-expand">
              <div className="flex items-center justify-between px-5 py-4">
                <div>
                  <div className="text-sm font-semibold text-gray-900">{singleDayLabelText}</div>
                  <div className="text-xs text-gray-500">Basculer sur une date unique</div>
                </div>
